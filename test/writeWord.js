...
describe('writeWord', () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(mockI2c, "writeWordAsync").callsFake(
      (device, cmd, word, cb) => {
        setImmediate(cb, null);
      }
    );
  ...
  it('writes word to register', (done) => {
    const addr = 0x1;
    const cmd = 0x2;
    const word = 0xaa55;

    i2c1.writeWord(addr, cmd, word, (err, word) => {
      ...
      assert(mockI2c.writeWordAsync.calledOnce);
      assert.strictEqual(mockI2c.writeWordAsync.firstCall.args.length, 4);
      const actualCmd = mockI2c.writeWordAsync.firstCall.args[1];
      assert.strictEqual(cmd, actualCmd);
      const actualWord = mockI2c.writeWordAsync.firstCall.args[2];
      assert.strictEqual(actualWord, actualWord);
  ...
  it('fails if /dev/i2c-<busNumber> not found', (done) => {
    ...
    i2c1.writeWord(addr, cmd, word, (err) => {
  ...
  afterEach(() => {
    ...
    mockI2c.writeWordAsync.restore();
