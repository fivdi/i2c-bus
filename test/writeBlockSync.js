...
describe('writeBlockSync', () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(mockI2c, 'writeBlockSync').callsFake(() => {});
  });
  ...
  it('writes block to register', () => {
    ...
    i2c1.writeBlockSync(addr, cmd, length, buffer);
    ...
    assert(mockI2c.writeBlockSync.calledOnce);
    assert.strictEqual(mockI2c.writeBlockSync.firstCall.args.length, 4);
    const actualCmd = mockI2c.writeBlockSync.firstCall.args[1];
    assert.strictEqual(cmd, actualCmd);
    const actualLength = mockI2c.writeBlockSync.firstCall.args[2];
    assert.strictEqual(length, actualLength);
    const actualBuffer = mockI2c.writeBlockSync.firstCall.args[3];
  ...
  it('returns this', () => {
    ...
    assert.strictEqual(i2c1, i2c1.writeBlockSync(addr, cmd, length, buffer));
  });


  afterEach(() => {
    mockI2c.setAddrSync.restore();
    mockI2c.writeBlockSync.restore();
