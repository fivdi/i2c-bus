...
describe('writeBlock', () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(mockI2c, "writeBlockAsync").callsFake(
  ...
  it('writes block to register', (done) => {
    ...
    i2c1.writeBlock(addr, cmd, length, buffer, (err, bytesWritten, buf) => {
      ...
      assert(mockI2c.writeBlockAsync.calledOnce);
      assert.strictEqual(mockI2c.writeBlockAsync.firstCall.args.length, 5);
      const actualCmd = mockI2c.writeBlockAsync.firstCall.args[1];
      assert.strictEqual(cmd, actualCmd);
      const actualLength = mockI2c.writeBlockAsync.firstCall.args[2];
      assert.strictEqual(length, actualLength);
      const actualBuffer = mockI2c.writeBlockAsync.firstCall.args[3];
  ...
  afterEach(() => {
    ...
    mockI2c.writeBlockAsync.restore();
